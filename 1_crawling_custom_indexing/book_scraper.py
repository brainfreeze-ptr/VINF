...
class GoodreadsScraper():
    ...
    def __init__(self, url_limit=10):
        ...
        self.timeout = 5  # used to wait for page to load completely
    ...
    def crawl(self):
        ...
        while self.url_limit > 0:
            ...
            # find other links
            found_sites = set(re.findall(r'\"(https://www\.goodreads\.com/book/show/.*?)\"', page_content))
        ...
        print(f'Saved: {self.saved_sites}, Queue size: {len(self.queue)}, books: {self.download_size/1000000:.2f} MB')
        self.save_state()
        driver.quit()

    def save_site(self, site, page_content):
        new_file_path = f"book/{re.findall(r'/(?:.(?!/))+$', site)[0]}"
    ...
    def crawlable(self, current_site, found_site):
        if re.search(r'\.com/.*/book', found_site):   # skip other language versions of the same book
            return False
    ...
    def print_state(self):
        print(f'Saved: {self.saved_sites}, Queue size: {len(self.queue)}, books: {self.download_size/1000000:.2f} MB', end='                       \r')


if __name__ == '__main__':
    gs = GoodreadsScraper(100)
    gs.crawl()
